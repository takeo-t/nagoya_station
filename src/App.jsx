...
const StationTitle = StationsData.StationsData;
const stations = [];
...
export const App = () => {
  ...
  const search = (value) => {
    ...
    const serchedStations = stations.filter(//フィルター関数は条件に一致する値のみ配列の中から取り出すことができる
      (station) => Object.values(station).filter(
        ...
        item !== undefined &&
        item !== null &&
        item.toString().toUpperCase().indexOf(value.toUpperCase()) !== -1
      ).length > 0
    );
  ...
  return (
    <>
    <h1>名鉄名古屋駅乗車位置検索</h1>
    <SerchedStations inputValue={inputValue} onChange={handleInputChange}/>
    <ShowStation />
  </>
  );
};